...
public class CampannaDAO {
	...
	public List<Campanna> listarCampannas(String FecIni, String FecFin,
		...
		String SELECT_CAMP = "SELECT TC.ROW_ID, TC.NAME,"
				+ "REPLACE(REPLACE(TC.OBJECTIVE,CHR(13),''),CHR(10),'')OBJECTIVE, "
				+ "TC.X_OWNER_BU, TC.CAMP_TYPE_CD,TC.CAMP_CAT_CD,TC.PRIORITY_CD,TC.PAR_SRC_ID,"
				+ "TC.STATUS_CD,TC.EXEC_APPR_STAT_CD, TC.MKTG_PLAN_ID,TC.X_HASHTAG, "
				+ "TO_CHAR(TC.PROG_START_DT,'YYYYMMDD')PROG_START_DT, "
				+ "TO_CHAR(TC.PROG_END_DT,'YYYYMMDD')PROG_END_DT, "
				+ "TC.BDGT_AMT, TU.LOGIN , TO_CHAR(TC.LAST_UPD,'YYYYMMDD')LAST_UPD, "
				+ "TC.SRC_NUM, TC.X_ID_CAMP "
				+ "FROM " + DBO + ".S_SRC TC, " + DBO + ".S_USER TU "
				+ "WHERE TC.LAST_UPD "
				+ "BETWEEN TO_DATE('"+FecIni+"','YYYYMMDD') "
				+ "AND TO_DATE('"+FecFin+"','YYYYMMDD') "
				+ "AND TC.LAST_UPD_BY = TU.ROW_ID(+) "
