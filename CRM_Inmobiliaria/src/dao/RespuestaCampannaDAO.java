...
public class RespuestaCampannaDAO {
	...
	public List<RespuestaCampanna> listarRespuestasCampannas(String FecIni,
			String FecFin, String cfg, Connection conn) {
		...
		String SELECT_CATRCAM = "SELECT DISTINCT TR.SRC_ID, TT.MKTG_OFFR_ID,"
				+ "TT.MEDIA_TYPE_CD, TR.PR_CON_ID, TR.CALL_LST_ID,"
				+ "TR.OUTCOME_CD,TR.COMM_METHOD, "
				+ "TR.MEET_LOC, TO_CHAR(TR.LAST_UPD,'YYYYMMDD')LAST_UPD, "
				+ "TO_CHAR(TR.COMM_DATE,'YYYYMMDD')COMM_DATE, "
				+ "TR.COMM_DATE "
				+ "FROM " + DBO + ".S_COMMUNICATION TR, " + "" + DBO
				+ ".S_DMND_CRTN_PRG TT, " + "" + DBO + ".S_SRC TC "
				+ "WHERE TR.LAST_UPD " + "BETWEEN TO_DATE('" + FecIni
				+ "','YYYYMMDD') " + "AND TO_DATE('" + FecFin
				+ "','YYYYMMDD') " + "AND TC.ROW_ID=TR.SRC_ID "
	...
	public List<RespuestaCampanna> listarRespuestasCampannasCtl(String FecIni,
			String FecFin, String cfg, Connection conn) {
		Properties prop = new Properties();
		InputStream configFile = null;
		try {
		...
		String SELECT_CATRCAMCTL = "SELECT "
				...
				+ FecIni + "','YYYYMMDD') " + "AND TO_DATE('" + FecFin
				+ "','YYYYMMDD') " + "AND  TC.ROW_ID=TR.SRC_ID "
				+ "AND TT.ROW_ID=TR.DCP_ID "
				+ "AND TC.SUB_TYPE ='MARKETING_CAMPAIGN' "
				+ "AND TC.CAMP_TYPE_CD='Campaign' "
